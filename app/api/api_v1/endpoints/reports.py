...
async def export_pptx_report(
    ...
    """
    Export client report as PowerPoint file using the new endpoint path
    that matches the frontend request.
    """
    logger.info(
        f"Processing PPTX export for client_id: {client_id}, dates: {start_date} to {end_date}"
    )
    ...
    if not user_has_access:
        logger.warning(
            f"User {current_user.email} attempted to access client {client_id} without permission"
        )
        raise HTTPException(
...
async def export_client_report(
    ...
    """
    logger.info(
        f"Processing report export for client_id: {client_id}, dates: {start_date} to {end_date}"
    )
...
def create_powerpoint_report(output_path, client_name, start_date, end_date):
    ...
    try:
        ...
        total_impressions = sum(
            campaign.get("metrics", {}).get("impressions", 0) for campaign in campaigns
        )
        total_clicks = sum(
            campaign.get("metrics", {}).get("clicks", 0) for campaign in campaigns
        )
        total_conversions = sum(
            campaign.get("metrics", {}).get("conversions", 0) for campaign in campaigns
        )
        total_spend = sum(
            campaign.get("metrics", {}).get("spend", 0) for campaign in campaigns
        )
        ...
        roi = (
            ((total_conversions * 100) - total_spend) / total_spend
            if total_spend > 0
            else 0
        )
        ...
        if not os.path.exists(template_path):
            ...
            # Try an alternative path for Vercel deployment
            alt_template_path = os.path.join("app", "templates", "report_template.pptx")
            if os.path.exists(alt_template_path):
                logger.info(f"Using alternative template path: {alt_template_path}")
                template_path = alt_template_path
            else:
                logger.error(
                    f"Alternative template path not found either: {alt_template_path}"
                )
                raise Exception(
                    f"Report template not found at: {template_path} or {alt_template_path}"
                )
