...
const AdminDashboard: React.FC = () => {
  ...
  if (user?.role !== 'admin') {
    console.log(`Admin Dashboard: User ${user?.email} has role ${user?.role}, not admin. Redirecting to dashboard`);
    return <Navigate to="/dashboard" replace />;
  }

  console.log(`Admin Dashboard: Admin user ${user?.email} authenticated successfully`);
...
const AdminDashboardContent: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const clientsResponse = await axios.get('http://localhost:8000/api/v1/clients', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      console.log("AdminDashboard: Clients data received:", clientsResponse.data);
      ...
      if (mountedRef.current) {
        ...
        setClients([
          { id: 1, name: 'Demo Client', campaign_keywords: 'marketing, demo', is_active: true }
        ]);
        setUsers([
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (loading) {
        console.log("AdminDashboard: Safety timeout triggered - forcing loading state to false");
        setLoading(false);
  ...
  const handleClientFormChange = useCallback((field: keyof typeof clientForm, value: string) => {
    setClientForm(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleUserFormChange = useCallback((field: keyof UserForm, value: string | number[]) => {
  ...
  const handleSaveClient = useCallback(async () => {
    ...
    try {
      ...
      console.log(`AdminDashboard: ${selectedClient ? 'Updating' : 'Creating'} client:`, clientForm);

      if (selectedClient) {
        await axios.put(
          `http://localhost:8000/api/v1/clients/${selectedClient.id}`, 
          clientForm,
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
        showNotification(`Client "${clientForm.name}" updated successfully`, 'success');
      } else {
        await axios.post(
        ...
        showNotification(`Client "${clientForm.name}" created successfully`, 'success');
      }
      await loadData();
      ...
      if (mountedRef.current) {
        ...
        setError('Failed to save client: ' + (error instanceof Error ? error.message : 'Unknown error'));
        showNotification('Failed to save client', 'error');
      }
    }
  }, [api, clientForm, selectedClient, loadData, handleCloseClientDialog, showNotification]);
    ...
    if (window.confirm('Are you sure you want to delete this client?')) {
      try {
        ...
        if (mountedRef.current) {
          ...
          setError('Failed to delete client: ' + (error instanceof Error ? error.message : 'Unknown error'));
  ...
  const handleSaveUser = useCallback(async () => {
    ...
    try {
      ...
      console.log(`AdminDashboard: ${selectedUser ? 'Updating' : 'Creating'} user:`, userForm);
      ...
      if (selectedUser) {
        ...
        showNotification(`User ${userForm.email} updated successfully`, 'success');
      } else {
        await axios.post(
        ...
        showNotification(`User ${userForm.email} created successfully`, 'success');
      }
      await loadData();
      ...
      if (mountedRef.current) {
        ...
        setError('Failed to save user: ' + (error instanceof Error ? error.message : 'Unknown error'));
        showNotification('Failed to save user', 'error');
      }
    }
  }, [selectedUser, userForm, loadData, handleCloseUserDialog, showNotification]);
  ...
  const handleAssignClient = useCallback(async (userId: number, clientId: number) => {
    if (!mountedRef.current) return;
    try {
      setError(null);
  ...
  const handleRemoveClient = useCallback(async (userId: number, clientId: number) => {
    if (!mountedRef.current) return;
    try {
      setError(null);
    ...
    return (
      <Container maxWidth="xl" sx={{ mt: 4 }}>
        ...
        <Alert severity="error" sx={{ mb: 3 }}>
          {error}
          <Button variant="text" color="inherit" onClick={loadData} sx={{ ml: 2 }}>
            Retry
          </Button>
        </Alert>
      </Container>
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      {user.clients.length > 0 ? 
                        clients
                          .filter(client => user.clients.includes(client.id))
              ...
              <TableBody>
                {clients.map((client) => (
                  <TableRow key={client.id}>
                    ...
                    <TableCell align="right">
                      <IconButton size="small" onClick={() => handleOpenClientDialog(client)}>
                        <EditIcon />
                      </IconButton>
                      <IconButton 
                        size="small" 
      ...
      <Dialog open={openClientDialog} onClose={handleCloseClientDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) => handleClientFormChange('campaign_keywords', e.target.value)}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseClientDialog} color="inherit">Cancel</Button>
      ...
      <Dialog open={openUserDialog} onClose={handleCloseUserDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="dense">
            ...
            <Select
              value={userForm.role}
              label="Role"
              onChange={(e) => handleUserFormChange('role', e.target.value as 'client_manager' | 'admin')}
            >
          ...
          <FormControl fullWidth margin="dense">
            ...
            <Select
              ...
              onChange={(e) => handleUserFormChange('clients', e.target.value as number[])}
            >
              {clients.map((client) => (
        ...
        <DialogActions>
          <Button onClick={handleCloseUserDialog} color="inherit">Cancel</Button>
