...
import { 
  ...
  CardActions, Button, Paper, CircularProgress, Chip,
  Divider, List, ListItem, ListItemText, ListItemIcon,
  ListItemButton, IconButton, Tooltip, Stack, Snackbar, Alert
} from '@mui/material';
import { 
...
const Dashboard = () => {
    ...
    try {
      ...
      setClients([
        { 
          ...
          name: 'Acme Corp', 
          campaigns: [
            { id: 1, name: 'Summer Promotion', platform: 'LinkedIn', status: 'Active' }
          ] 
        },
        { 
          id: 2, 
          name: 'TechStart', 
          campaigns: [
            { id: 2, name: 'Product Launch', platform: 'Facebook', status: 'Scheduled' }
          ] 
        }
      ]);
      } finally {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (loading) {
        console.log("Dashboard: Safety timeout triggered - forcing loading state to false");
        setLoading(false);
          ...
          <Paper elevation={2} sx={{ p: 2 }}>
            ...
            <List component="nav">
              ...
              {isAdmin ? (
                ...
                <ListItemButton onClick={() => handleFeatureClick('Client Management')}>
                  <ListItemIcon>
                    <BusinessIcon color="primary" />
                  </ListItemIcon>
          ...
          {isAdmin && (
            <Box sx={{ mb: 4 }}>
              <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'primary.light', color: 'primary.contrastText' }}>
                <Typography variant="h6" gutterBottom>Welcome to the Admin Dashboard</Typography>
                <Typography variant="body2">
                  As an administrator, you have access to user and client management features.
                  Use the sidebar links or visit the Admin Dashboard for more options.
                </Typography>
              </Paper>
            </Box>
          )}
            ...
            <Paper sx={{ p: 3, mb: 4 }}>
              <Box sx={{ textAlign: 'center', py: 2 }}>
                ...
                <Typography variant="body1" paragraph>
                  Access detailed performance metrics for all your assigned clients.
                  View campaign statistics, engagement data, and ROI metrics.
                </Typography>
                <Button
                  variant="contained"
                  color="primary"
